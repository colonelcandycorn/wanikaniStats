...
impl CompleteUserInfo {
    ...
    pub fn get_total_reading_count(&self) -> i32 {
        self.get_total_correct_reading_count() +
        self.get_total_incorrect_reading_count()
    }

    pub fn get_total_meaning_count(&self) -> i32 {
        self.get_total_correct_meaning_count() +
        self.get_total_incorrect_meaning_count()
    }
    ...
    pub fn get_total_reading_accuracy(&self) -> f64 {
        (self.get_total_correct_reading_count() as f64 / self.get_total_reading_count() as f64) * 100.0
    }

    pub fn get_total_meaning_accuracy(&self) -> f64 {
        (self.get_total_correct_meaning_count() as f64 / self.get_total_meaning_count() as f64) * 100.0
    }

    pub fn get_radical_meaning_accuracy(&self) -> f64 {
        let radical_count = (self.radical_stats.num_of_meaning_correct + self.radical_stats.num_of_meaning_incorrect) as f64;

        (self.radical_stats.num_of_meaning_correct as f64 / radical_count) * 100.0
    }

    pub fn get_kanji_reading_accuracy(&self) -> f64 {
        let kanji_count = (self.kanji_stats.num_of_reading_correct + self.kanji_stats.num_of_reading_incorrect) as f64;

        (self.kanji_stats.num_of_reading_correct as f64 / kanji_count) * 100.0
    } 

    pub fn get_kanji_meaning_accuracy(&self) -> f64 {
        let kanji_count = (self.kanji_stats.num_of_meaning_correct + self.kanji_stats.num_of_meaning_incorrect) as f64;

        (self.kanji_stats.num_of_meaning_correct as f64 / kanji_count) * 100.0
    }

    pub fn get_vocab_reading_accuracy(&self) -> f64 {
        let vocab_count = (self.vocab_stats.num_of_reading_correct + self.vocab_stats.num_of_reading_incorrect + self.kana_stats.num_of_reading_correct + self.kana_stats.num_of_reading_incorrect) as f64;
        let vocab_correct = (self.vocab_stats.num_of_reading_correct + self.kana_stats.num_of_reading_correct) as f64;

        (vocab_correct / vocab_count) * 100.0
    }  

    pub fn get_vocab_meaning_accuracy(&self) -> f64 {
        let vocab_count = (self.vocab_stats.num_of_meaning_correct + self.vocab_stats.num_of_meaning_incorrect + self.kana_stats.num_of_meaning_correct + self.kana_stats.num_of_meaning_incorrect) as f64;
        let vocab_correct = (self.vocab_stats.num_of_meaning_correct + self.kana_stats.num_of_meaning_correct) as f64;

        (vocab_correct / vocab_count) * 100.0
    }

    pub fn get_kanji_total_accuracy(&self) -> f64 {
        let kanji_count = (self.kanji_stats.num_of_meaning_correct + self.kanji_stats.num_of_meaning_incorrect + self.kanji_stats.num_of_reading_correct + self.kanji_stats.num_of_reading_incorrect) as f64;
        let kanji_correct = (self.kanji_stats.num_of_meaning_correct + self.kanji_stats.num_of_reading_correct) as f64;

        (kanji_correct / kanji_count) * 100.0
    }

    pub fn get_vocab_total_accuracy(&self) -> f64 {
        let vocab_count = (self.vocab_stats.num_of_meaning_correct + self.vocab_stats.num_of_meaning_incorrect + self.vocab_stats.num_of_reading_correct + self.vocab_stats.num_of_reading_incorrect + self.kana_stats.num_of_meaning_correct + self.kana_stats.num_of_meaning_incorrect + self.kana_stats.num_of_reading_correct + self.kana_stats.num_of_reading_incorrect) as f64;
        let vocab_correct = (self.vocab_stats.num_of_meaning_correct + self.vocab_stats.num_of_reading_correct + self.kana_stats.num_of_meaning_correct + self.kana_stats.num_of_reading_correct) as f64;
...
impl CompleteUserInfoBuilder {
    ...
    pub fn build(self) -> Result<CompleteUserInfo, MissingSubjectError> {
        ...
        let vocab_stats = self.get_subject_type_stats(SubjectType::Vocabulary).unwrap();
        let kana_stats = self.get_subject_type_stats(SubjectType::KanaVocabulary).unwrap();
        ...
        let vocab_learned = self
            .get_num_of_passed(SubjectType::Vocabulary)
            .unwrap();
        let kana_learned = self
            .get_num_of_passed(SubjectType::KanaVocabulary)
            .unwrap();
